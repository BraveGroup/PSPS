...
class MaskDETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is DETRHead):
            ...
            class_weight = torch.ones(num_thing_classes + 1) * class_weight
            # set background class as the last indice
            class_weight[num_thing_classes] = bg_cls_weight
        ...
        if self.loss_cls.use_sigmoid:
            self.cls_out_channels = num_thing_classes
        else:
            self.cls_out_channels = num_thing_classes + 1
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        hs, init_reference, inter_references, \
            ...
            memory, memory_pos, memory_mask, query_pos = self.transformer(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
                    mlvl_positional_encodings,
        ...
        last_feat_len = hw_lvl[-1][0] * hw_lvl[-1][1] # We do not use the last scale
        outputs_for_mask = (
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         ...
         gt_inds_list) = multi_apply(
             self._get_target_single, cls_scores_list, bbox_preds_list,
             gt_bboxes_list, gt_labels_list, img_metas, gt_bboxes_ignore_list)
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))
        return (labels_list, label_weights_list, bbox_targets_list,
                bbox_weights_list, num_total_pos, num_total_neg, pos_inds_list, gt_inds_list)
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            det_labels = indexes % self.num_thing_classes
            bbox_index = indexes // self.num_thing_classes
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        *outs, outs_for_mask = self.forward(feats, img_metas)
